...
class DigitalGiftsController < ApplicationController
  ...
  def webhook
    tremendous_id = case params[:event]
                    when /^PAYOUTS/
                      params[:payload][:meta][:reward][:id]
                    when /^ORDER/
                      params[:payload][:resource][:id]
                    else
                      params.dig(:payload, :meta, :reward, :id) || params.dig(:payload, :resource, :id) # possibly?
  ...
  def validate_tremendous_request
    ...
    expected_signature = OpenSSL::HMAC.hexdigest(
      OpenSSL::Digest.new(algorithm), Rails.application.credentials.tremendous[:webhook], request.body.read
    )
